...
namespace Scheme.NET.Numbers {
    ...
    public struct BigRational : IComparable, IComparable<BigRational>, IDeserializationCallback, IEquatable<BigRational>, ISerializable {
        ...
        public string ToDecimalString(int digits = 10)
        {
            BigInteger r;
            var n = BigInteger.DivRem(Numerator, Denominator, out r);
            var intPart = BigInteger.Abs(n).ToString();
            var remainder = new BigRational(BigInteger.Abs(r), Denominator);
            var shiftedRemainder = remainder * BigInteger.Pow(10, digits);
            var decPart = (shiftedRemainder.Numerator / shiftedRemainder.Denominator).ToString();

            if (decPart.Length < digits)
                decPart = decPart.PadRight(digits, '0');


            if ((shiftedRemainder.Numerator % shiftedRemainder.Denominator).IsZero)
            {
        ...
        public static BigRational Factor(BigRational x)
        {
            if (x.Denominator == 1)
                return x;

            BigInteger factor = BigInteger.GreatestCommonDivisor(x.Numerator, x.Denominator);
            var n = x.Numerator / factor;
            var d = x.Denominator / factor;
            return new BigRational(n, d);
        }

        public static bool IsEven(BigRational x)
        {
            var f = Factor(x);
            if (f.Denominator != 1)
                return false;
            return f.Numerator.IsEven;
        }

        public static bool IsOdd(BigRational x)
        {
            var f = Factor(x);
            if (f.Denominator != 1)
                return false;
        ...
        {
            var quotient = BigInteger.Divide(x.Numerator, x.Denominator);
            BigInteger remainder = BigIntegerHelper.Modulo(x.Numerator, x.Denominator);

            if (remainder.IsZero || quotient.Sign > 0)
                return quotient;
            else
                return quotient - 1;
        }

        public static BigRational Ceil(BigRational x)
        {
            var quotient = BigInteger.Divide(x.Numerator, x.Denominator);
            BigInteger remainder = BigIntegerHelper.Modulo(x.Numerator, x.Denominator);
        ...
        public static BigRational Round(BigRational x)
        {
            if (x.Sign == 0)
                return x;

            BigRational value = x.GetFractionPart();

            if (x.Sign < 0)
            {
                if (value <= new BigRational(-1, 2))
                    return BigRational.Floor(x);
                return BigRational.Ceil(x);
            }
            else
            {
                if (value < new BigRational(1, 2))
        ...
        public static BigRational Truncate(BigRational x)
        {
            var numerator = x.Numerator - BigInteger.Remainder(x.Numerator, x.Denominator);
            return new BigRational(numerator, x.Denominator);
        }

        public static BigRational Modulo(BigRational r1, BigRational r2)
        {
            return new BigRational(
                BigIntegerHelper.Modulo((r1.m_numerator * r2.Denominator), (r1.Denominator * r2.m_numerator)), 
        ...
        public static BigRational Parse(string value)
        {
            ...
            if (pos < 0)
            {
                ...
                return Factor((new BigRational(numerator)));
            }
            else
            {
                ...
                return Factor(new BigRational(numerator, denominator));
            }
        }
        ...
        // ----- SECTION: explicit conversions from BigDecimal to numeric base types  ----------------*
        ...
        // ----- SECTION: implicit conversions from numeric base types to BigDecimal  ----------------*
